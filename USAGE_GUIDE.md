...
1. Откройте:
   - `/admin/payments`
2. Создайте свой шаблон в блоке `Template Builder`:
   - `Template Name`;
   - `Price (USD)`;
   - `TTL`;
   - `USDT Network (default)` — сеть, которая будет использоваться при выборе валюты `USDT`;
   - `Description`.
3. Сохраните шаблон (`Create Template`).
4. В карточке шаблона, для быстрого выпуска инвойса:
   - выберите только `Currency` (`USDT` или `BTC`);
   - поле `Network` заполнится автоматически.
5. Нажмите `Create Invoice`.
6. Система откроет карточку платежа `/admin/payments/<id>`.

Важно:
1. Сумма в USD берется из сохраненного шаблона.
2. Для `USDT` сеть берется из `USDT Network` шаблона.
3. Для `BTC` сеть всегда `btc`.
4. Любой шаблон можно отредактировать (название, цену, TTL, описание, активность).
...
Показывает:
1. конструктор шаблонов быстрых платежей;
2. сохраненные шаблоны (редактирование названия/цены/TTL/описания/активности);
3. быстрый выпуск инвойса из шаблона через выбор валюты;
4. последние платежи (реестр) с переходом в карточку.
